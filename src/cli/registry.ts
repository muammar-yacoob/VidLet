...
export async function addRegistryKey(
  ...
  try {
    ...
    const shouldIgnore = ignoredErrors.some((e) => message.toLowerCase().includes(e.toLowerCase()));
    if (!shouldIgnore) {
...
async function registerMenuForExtension(
  ...
  const extensionTools = toolConfigs.filter((t) => t.extensions.includes(extension));
 ...
 */
async function unregisterMenuForExtension(extension: string): Promise<RegistrationResult[]> {
  const results: RegistrationResult[] = [];
  const basePath = getMenuBasePath(extension);
...
export async function registerAllTools(): Promise<RegistrationResult[]> {
  ...
  for (const extension of getAllExtensions()) {
    const extResults = await registerMenuForExtension(extension, iconsDir, launcherPath);
