...
export function registerInstallCommand(program: Command): void {
  program
    ...
    .action(async () => {
      ...
      console.log(
        `  ${fmt.cyan('vidlet compress')} video.mp4 -b 2000    ${fmt.dim('# reduce to 2000 kbps')}`
      );
      console.log(
        `  ${fmt.cyan('vidlet togif')} video.mp4 -f 15 -w 480  ${fmt.dim('# gif at 15fps, 480px wide')}`
      );
      console.log(
        `  ${fmt.cyan('vidlet mkv2mp4')} video.mkv             ${fmt.dim('# convert mkv to mp4')}`
      );
      console.log(
        `  ${fmt.cyan('vidlet shrink')} video.mp4 -t 59        ${fmt.dim('# speed up to 59 seconds')}`
      );
      console.log(
        `  ${fmt.cyan('vidlet loop')} video.mp4 -s 5 -e 15     ${fmt.dim('# trim between 5s and 15s')}`
      );
      console.log(
        `  ${fmt.cyan('vidlet thumb')} video.mp4 cover.jpg     ${fmt.dim('# embed cover.jpg as thumbnail')}`
      );
      console.log();
    });
}
