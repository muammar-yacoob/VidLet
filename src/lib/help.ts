...
export function showHelp(): void {
  ...
  console.log(
    `    ${$} vidlet ${cmd('mkv2mp4')} ${arg('clip.mkv')} ${opt('-y')}          ${comment('# Use defaults')}`
  );
  console.log(
    `    ${$} vidlet ${cmd('shrink')} ${arg('clip.mp4')} ${opt('-t 30')}        ${comment('# Fit to 30 seconds')}`
  );
  console.log(
    `    ${$} vidlet ${cmd('thumb')} ${arg('clip.mp4 cover.png')}     ${comment('# Embed thumbnail')}`
  );
  console.log(
    `    ${$} vidlet ${cmd('loop')} ${arg('clip.mp4')} ${opt('-s 1 -e 3')}      ${comment('# Loop 1s to 3s')}`
  );
