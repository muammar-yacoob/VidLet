...
export async function portraitMultiSegment(options: PortraitMultiSegmentOptions): Promise<string> {
  ...
  try {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      console.log(
        fmt.dim(
          `  Segment ${i + 1}/${segments.length}: ${seg.startTime.toFixed(2)}s - ${seg.endTime.toFixed(2)}s (crop: ${(seg.cropX * 100).toFixed(0)}%)`
        )
      );
    ...
    if (useTransitions) {
      ...
      for (let i = 0; i < segmentFiles.length - 1; i++) {
        ...
        videoFilters.push(
          `${inputLabel}[${i + 1}:v]xfade=transition=${transition}:duration=${td}:offset=${offset.toFixed(3)}${outputLabel}`
        );
